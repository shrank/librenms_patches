    ...
    <div class="panel panel-default panel-condensed">
        ...
        <div class="table-responsive">
            <table id="sensors" class="table table-hover table-condensed"
                   data-url="{{ route('table.sensorsalert') }}" data-params="class={{ $metric }}">
                <thead>
                <tr>
    ...
    <script>
      $(document).ready(function(){
          ...
          $('.actionBar').append('<div class="pull-left">\
              <label for="sensor-status-{{ $id }}" class="control-label">{{ __('Status') }}:</label>\
              <select class="form-control" name="sensor-status" id="sensor-status-{{ $id }}">\
        ...
        $("#sensor-status-{{ $id }}").on("change", function() {
        ...
        var grid = $("#sensors").bootgrid({
            ...
            post: function () {
                let status = $("#sensor-status-{{ $id }}").val()
                if(!status) {
                  status = "alert";
                }
                return {
