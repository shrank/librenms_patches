...
class RunAlerts
{
    ...
    {
        foreach (AlertUtil::loadAlerts('alerts.state = ? && alerts.open = ?' , [AlertState::ACKNOWLEDGED, AlertState::ACTIVE]) as $alert) {
    ...
    {
        foreach (AlertUtil::loadAlerts('alerts.state != ? && alerts.open = 1', [AlertState::ACKNOWLEDGED]) as $alert) {
            $noiss = false;
            $noacc = false;
            $updet = false;
