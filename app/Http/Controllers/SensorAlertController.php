...
{
    public function index(Request $request, string $metric = 'all', string $legacyview = ''): View
    {
    ...
    private function getMetrics(Request $request): array
    {
        $metrics = [
            'all' => [
                'text' => __('All'),
                'link' => route('sensoralert.index', $request->all() + ['metric' => 'all']),
                'icon' => 'fa-bell',
            ],
            'mempool' => [
                'text' => __('Memory'),
                'link' => route('sensoralert.index', $request->all() + ['metric' => 'mempool']),
                'icon' => 'fa-memory',
            ],
            'processor' => [
                'text' => __('Processor'),
                'link' => route('sensoralert.index', $request->all() + ['metric' => 'processor']),
                'icon' => 'fa-microchip',
            ],
            'storage' => [
                'text' => __('Storage'),
                'link' => route('sensoralert.index', $request->all() + ['metric' => 'storage']),
                'icon' => 'fa-hdd',
            ],
            'diskio' => [
                'text' => __('Disk I/O'),
                'link' => route('sensoralert.index', $request->all() + ['metric' => 'diskio']),
                'icon' => 'fa-hdd',
            ],
            'printer-supply' => [
                'text' => __('sensors.printer-supply.long'),
                'link' => route('sensoralert.index', $request->all() + ['metric' => 'printer-supply']),
                'icon' => 'fa-print',
            ],
        ];
        ...
        foreach ($sensors_menu as $types) {
            foreach ($types as $entry) {
                ...
                $metrics[$class] = [
                    ...
                    'link' => route('sensoralert.index', $request->all() + ['metric' => $class]),
                    'icon' => 'fa-' . $entry['icon'],
                ];
            }
        }
